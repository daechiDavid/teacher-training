...
export function App() {
  ...
  useEffect(() => {
    return () => {
      if (currentDocument) URL.revokeObjectURL(currentDocument.url);
    };
  }, [currentDocument]);
  ...
  async function connectGoogle() {
    setError(null);
    setNotice("브라우저에서 Google 로그인을 완료한 뒤 앱으로 돌아오세요.");
    setBusy(true);
    try {
  ...
  async function loadCompletionDocumentSource() {
    ...
    setBusy(true);
    try {
      const spreadsheetId = extractSpreadsheetId(completionSheetForm.spreadsheetUrl, "연수 신청자 Google Sheet");
      ...
      if (parsed.missingHeaders.length) {
        throw new Error(`연수 신청자 Google Sheet 필수 헤더가 없습니다: ${parsed.missingHeaders.join(", ")}`);
      }
      ...
      setDocumentSourceFile({
        name: `Google Sheet: ${spreadsheetId} / ${sheetName}`,
  ...
  function validateDocumentBasics() {
    if (!documentRows.length) {
      setError("연수 신청자 Google Sheet를 먼저 불러오세요.");
      return false;
    }
  ...
  async function createCurrentDocument() {
    ...
    if (documentWorkflowStep === "doc2" && !zoomRows.length) {
      setError("2번 문서 생성을 위해 Zoom 접속기록 엑셀을 먼저 불러오세요.");
      return;
    }
    if (documentWorkflowStep === "doc3" && !zoomRows.length) {
      setError("3번 문서 생성을 위해 2번 문서의 Zoom 접속기록을 먼저 반영하세요.");
      return;
    }
    if (documentWorkflowStep === "doc5" && !evaluationSummary) {
      setError("5번 평가서 생성을 위해 평가 Google Sheet를 먼저 불러오세요.");
      return;
    }
    if (documentWorkflowStep === "doc7" && !summaryRows.length) {
      setError("7번 명단 생성을 위해 3번 종합 출결자료를 먼저 작성하세요.");
      return;
    }
    setBusy(true);
    try {
      ...
      setError(null);
      setNotice(`${built.label} 다운로드 파일을 생성했습니다. 다운로드 후 다음 문서로 이동하세요.`);
    } catch (createError) {
  ...
  async function buildDocumentForCurrentStep() {
    ...
    if (documentWorkflowStep === "doc7") {
      ...
      return {
        label: "7. 직무연수 이수자 명단",
        filename: buildAttendanceFilename(documentForm, "7. 직무연수 이수자 명단", "xlsx"),
  ...
  async function loadZoomFile(file: File | null) {
    if (!file) return;
    if (!isValidTimeText(documentForm.startTime) || !isValidTimeText(documentForm.endTime)) {
      setError("Zoom 접속기록을 불러오기 전에 연수 시작시간과 종료시간을 HH:mm 형식으로 입력하세요.");
      return;
    }
    setError(null);
    setBusy(true);
    try {
      ...
      clearCurrentDocument();
      setNotice("Zoom 접속기록을 2번 문서 미리보기에 반영했습니다.");
    } catch (zoomError) {
  ...
  async function loadEvaluationSheet() {
    ...
    setBusy(true);
    try {
      const spreadsheetId = extractSpreadsheetId(evaluationSheetForm.spreadsheetUrl, "평가 Google Sheet");
      const gid = extractGoogleSheetGid(evaluationSheetForm.spreadsheetUrl);
      const rawRows = await readGoogleSheetValues(spreadsheetId, gid);
      setEvaluationSummary(parseEvaluationRows(rawRows));
      clearCurrentDocument();
      setNotice("평가 Google Sheet를 5번 평가서 미리보기에 반영했습니다.");
  ...
  async function saveBaseSettings() {
    ...
    setBusy(true);
    try {
      const spreadsheetId = extractSpreadsheetId(googleForm.spreadsheetUrl, "전체명단 Google Sheet");
  ...
  async function loadCompletionFromGoogleSheet() {
    ...
    setBusy(true);
    try {
      const spreadsheetId = extractSpreadsheetId(completionSheetForm.spreadsheetUrl, "이수자 Google Sheet");
      const gid = extractGoogleSheetGid(completionSheetForm.spreadsheetUrl);
      const rawRows = await readGoogleSheetValues(spreadsheetId, gid);
      const parsed = parseCompletionWorkbookRows(rawRows);
      if (parsed.missingHeaders.length) {
        throw new Error(`이수자 Google Sheet 필수 헤더가 없습니다: ${parsed.missingHeaders.join(", ")}`);
      }
      const normalized = normalizeCompletionRows(parsed.rows);
      setCompletionRows(normalized);
      setCompletionFile({
        name: `Google Sheet: ${spreadsheetId}${gid == null ? "" : ` / gid ${gid}`}`,
        rowCount: normalized.length,
        missingHeaders: parsed.missingHeaders,
      });
  ...
  async function loadRosterFromGoogle() {
    ...
    if (parsed.missingHeaders.length) {
      throw new Error(`전체명단 Google Sheet 필수 헤더가 없습니다: ${parsed.missingHeaders.join(", ")}`);
    }
    ...
    setRosterFile({
      name: `Google Sheet: ${googleStatus?.sheet_name || "명단"}`,
      rowCount: normalized.length,
      missingHeaders: parsed.missingHeaders,
    });
  }
  ...
  async function reloadRosterForReview() {
    ...
    try {
      await loadRosterFromGoogle();
      setNotice("전체명단 Google Sheet를 다시 불러왔습니다.");
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : String(loadError));
    } finally {
      setBusy(false);
  ...
  async function generateUploadAndRecord() {
    ...
    if (!googleStatus?.configured || !googleStatus.authenticated) {
      setError("Google 로그인과 기본 설정을 먼저 완료하세요.");
      return;
    }
    ...
    setIssueProgress({
      ...
      label: "PDF 생성과 Google Drive 업로드를 준비 중입니다.",
    });
    setBusy(true);
    try {
      ...
      for (const [index, result] of eligibleResults.entries()) {
        ...
        const personKey = `${result.completion.name}_${result.completion.phone}`;
        ...
        try {
          setIssueProgress({
            current: index,
            total: eligibleResults.length,
            label: `${result.completion.name} PDF 생성 중`,
          });
          if (!folder) {
          ...
          setIssueProgress({
            current: index,
            total: eligibleResults.length,
            label: `${result.completion.name} PDF 업로드 중`,
          });
        ...
        <section className="workflow-stage">
          ...
          {step === "settings" ? (
            <StepCard
              icon={<Settings />}
              title="기본 설정"
              description="전체명단 Google Sheet URL과 영수증 업로드 Google Drive 폴더 URL을 한 번만 저장합니다."
            >
          ...
          {step === "upload" ? (
            <StepCard
              ...
              title="이수자 명단 불러오기"
              description="이수자 Google Sheet URL을 입력하면 저장된 전체명단 Google Sheet와 자동 대조합니다."
            >
              ...
              <FileStatus title="이수자 Google Sheet" state={completionFile} />
          ...
          {step === "review" ? (
            ...
            >
              ...
              <FileStatus title="전체명단 Google Sheet" state={rosterFile} />
          ...
          {step === "issue" ? (
            <StepCard
              icon={<FolderUp />}
              title="업로드 및 기록"
              description="첫 번째 대상자의 샘플 영수증을 확인한 뒤 전체 PDF 업로드와 전체명단 기록을 실행합니다."
            >
              {issueCompletion ? (
                ...
                <>
                  <div className="notice">
                    <UploadCloud size={18} />
                    업로드된 개별 PDF는 링크가 있는 사용자에게 뷰어 권한으로 공유됩니다.
                  </div>
    ...
    <header className="topbar">
      <div className="brand">
        ...
        <div>
          ...
          <p>{authenticated ? "전체명단 대조, PDF 샘플 확인, 업로드 기록을 단계별로 처리합니다." : "Google 로그인 후 업무 화면을 시작합니다."}</p>
        </div>
      </div>
...
function LoginGate({
  ...
  return (
    <section className="login-gate">
      ...
      {!status?.client_id ? <p className="muted">관리자 OAuth 설정이 필요합니다.</p> : null}
    </section>
  );
}
  ...
  return (
    <section className="workflow-stage">
      ...
      <StepCard
        icon={<FileSpreadsheet />}
        title="이수 서류 작성"
        description="연수 신청자 Google Sheet, Zoom 접속기록, 평가 Google Sheet를 기준으로 HWPX 4개와 XLSX 1개를 생성합니다."
      >
        ...
        <FileStatus title="연수 신청자 Google Sheet" state={sourceFile} />
        {sourceRows.length ? (
          <>
            ...
            {workflowStep === "done" ? <p className="notice">5개 문서 다운로드 흐름을 완료했습니다.</p> : (
              <DocumentStepActions
                busy={busy}
                step={workflowStep}
  ...
  return (
    <div className="document-actions">
      <button className="primary-action" type="button" onClick={onCreate} disabled={busy || step === "done"}>
        {documentStepLabel(step)} 다운로드 파일 만들기
      </button>
      {document ? (
        <a className="download-action" href={document.url} download={document.filename} onClick={onDownloaded}>
          <Download size={17} />
          {document.label} 다운로드
        </a>
      ) : null}
...
function CompletionPreviewTable({ rows }: { rows: CompletionDocumentRow[] }) {
  if (!rows.length) {
    return <p className="muted">서류 만들기를 클릭하면 생성될 xlsx 내용이 미리보기로 표시됩니다.</p>;
  }
...
function SelectOX({ value, busy, onChange }: { value: "O" | "X"; busy: boolean; onChange: (value: "O" | "X") => void }) {
  return (
    <select value={value} onChange={(event) => onChange(event.target.value as "O" | "X")} disabled={busy}>
...
function ZoomControls({
  ...
  return (
    <section className="sub-panel">
      ...
      <label className="file-picker">
        Zoom 접속기록 엑셀 업로드
        <input type="file" accept=".xlsx,.xls" onChange={(event) => onFileChange(event.target.files?.[0] ?? null)} disabled={busy} />
      </label>
      <FileStatus title="Zoom 접속기록" state={zoomFile} />
      {rows.length ? (
        <div className="table-wrap compact-preview">
          <table>
            <thead>
...
function GoogleUrlSettingsForm({
  ...
  return (
    <div className="settings-form">
      <label>
        <span>전체명단 Google Sheet URL</span>
        <input
          value={value.spreadsheetUrl}
      ...
      <label>
        <span>영수증 업로드 Google Drive 폴더 URL</span>
        <input
...
function CompletionSheetUrlForm({
  ...
  return (
    <div className="settings-form">
      <label>
        <span>연수 신청자 Google Sheet URL</span>
        <input
          value={value.spreadsheetUrl}
          onChange={(event) => onChange({ spreadsheetUrl: event.target.value })}
          placeholder="https://docs.google.com/spreadsheets/d/.../edit#gid=0"
      ...
      <p className="muted">URL에 gid가 있으면 해당 탭을, 없으면 첫 번째 탭을 읽습니다.</p>
    </div>
  );
}
...
function FileStatus({ title, state }: { title: string; state: LoadedFileState | null }) {
  ...
  return (
    <div className={state.missingHeaders.length ? "file-status blocked" : "file-status ok"}>
      ...
      <span>{state.rowCount.toLocaleString()}행</span>
      {state.missingHeaders.length ? (
        <span>누락 헤더: {state.missingHeaders.join(", ")}</span>
      ) : (
        <span>필수 헤더 확인 완료</span>
      )}
    </div>
  );
}
...
function SummaryGrid({ summary }: { summary: { completed: number; eligible: number; manual: number; excluded: number } }) {
  return (
    <div className="summary-grid">
      <Metric label="이수 필터 통과" value={summary.completed} />
      <Metric label="실제 발급 인원" value={summary.eligible} />
...
function IssueList({
  issues,
  emptyMessage = "전체명단 발급 기록 정합성 오류가 없습니다.",
}: {
  issues: ValidationIssue[];
  emptyMessage?: string;
}) {
...
function ResultTable({ results }: { results: ReturnType<typeof matchRecipients> }) {
  if (!results.length) {
    return <p className="muted">이수자 명단을 업로드하면 발급 대상자 목록이 표시됩니다.</p>;
  }
...
function extractSpreadsheetId(value: string, label: string): string {
  ...
  throw new Error(`${label} URL에서 Spreadsheet ID를 찾지 못했습니다.`);
}
...
function extractDriveFolderId(value: string): string {
  ...
  if (/^[a-zA-Z0-9-_]{20,}$/.test(trimmed)) return trimmed;
  throw new Error("Google Drive 폴더 URL에서 폴더 ID를 찾지 못했습니다.");
}
